...
function applyFilter() {
    state.filtered = state.requests.filter(req => {
        ...
        if (state.search) {
            ...
            const haystack = (req.path + ' ' + req.method + ' ' +
...
function renderPreviewTab(req) {
    ...
    if (!req.response_body || (typeof req.response_body === 'object' && Object.keys(req.response_body).length === 0)) {
        div.innerHTML = '<span style="color:#555">No response data' + (!req.status ? ' (pending...)' : '') + '</span>';
        return;
    }

    div.innerHTML = buildJsonTree(req.response_body, null);
}
